...
fatal_plot <- ggplot() + 
  geom_bar( data = healt_impact_fatalities[1:10,], aes( x = reorder(EVTYPE, -EVTYPE), y = fatalities, fill = interaction(fatalities, EVTYPE)), stat = "identity", show.legend=F) +
  xlab("Harmful Events") + 
  ylab("No. of fatailities") + 
  ggtitle("Top 5 weather events causing fatalities")

grid.arrange(fatal_plot)